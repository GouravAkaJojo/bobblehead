...
for path in sys.argv[1:]:
    ...
    for i in range(9):
        ...
        # Measure the shoulders on the CHARACTER, not on whatever is lowest in the
        # cell. This band is taken upward from the lowest opaque pixel, so a single
        # stray speck below the bust drags it off the body and the width collapses to
        # the speck. A detailed style throws off plenty of specks: one sheet measured
        # 103% spread on widths of [256, 257, 255, 73, 62, 70, 264, 263, 264], and the
        # three odd ones were flecks, not shoulders. Cleaned, the same sheet is 3.9%.
        # The build already works on the largest connected component, so matching it
        # here is what makes this a prediction of the build rather than a lottery.
        lab, n = ndimage.label(op)
        main = op
        if n > 1:
            main = lab == int(np.argmax(ndimage.sum(op, lab, range(1, n + 1)))) + 1
        ys, xs = np.where(main)
        band = ys.max() - int(W * 0.05)
        sy, sx = np.where(main[band:, :])
