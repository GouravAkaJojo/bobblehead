...
CENTER = 16   # the resting cell of a 6x3 ring atlas


def cells(path):
    """Every cell of a sheet, square tiles, whatever the grid.

    The directions atlas is a 6x3 ring of sixteen head angles plus a resting cell and a
    padding cell; the expressions atlas is still 3x3. Deriving the columns from the
    aspect ratio rather than assuming three keeps one reader working for both.
    """
    im = Image.open(path).convert('RGBA')
    cols = round(im.size[0] / im.size[1] * 3)
    t = im.size[0] // cols
    rows = im.size[1] // t
    return t, [np.array(im.crop(((i % cols) * t, (i // cols) * t,
...
for name in sys.argv[1:]:
    ...
    scale = DISPLAY / tile
    ref = lower_band(body(directions[CENTER if len(directions) > 9 else 4]))
...
# The blink ring stands in for the directions ring cell for cell, so any drift
# between them is a head twitch every time the character blinks while tracking.
for name in sys.argv[1:]:
    path = os.path.join(DEST, f'{name}-blink.webp')
    ...
    worst, where = 0.0, -1
    for i in range(16):
        shift = max(abs(dy) for _, dy in [best_shift(lower_band(body(directions[i])),
                                                     lower_band(body(blinks[i])))]) * scale
        if shift > worst:
            worst, where = shift, i
    spot = 'no cell drifts' if where < 0 else f'worst at cell {where}'
    print(f'{name:11} blink ring: worst per-cell drift {worst:.2f} px, {spot}'
          f'  ({"ok" if worst <= 2 else "TOO MUCH"})')
