...
def main():
    ...
    parser.add_argument('--style', default='colour',
                        choices=['colour', 'anime', 'jjba', 'ink', 'sketch', 'riso', 'paper', 'pixel'],
    ...
    for attempt in range(args.retries + 1):
        ...
        # The in-between sheet is optional: without it the build fills the odd cells
        # from sheet A and the mascot runs at eight directions instead of sixteen.
        # A sheet that exists but screens badly is worse than none, so drop it.
        for extra, flag in (('directions-b', 'between'), ('directions-c', 'lower')):
            path = os.path.join(ROOT, 'characters', args.name, f'{extra}.png')
            if os.path.exists(path) and not screen(args.name, extra):
                print(f'  -> {extra} is unusable, setting it aside; '
                      f'rerun with --only {flag} to redraw just that one')
